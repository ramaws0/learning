from autogen import ConversableAgent
import time 

import os
API_KEY = os.getenv("API_KEY")

print("\n📖 LESSON 1: Configuring the Agent System For First Time")
print("-" * 40)

# =============================================================================
# LESSON 1: Configuring the Agent System
# =============================================================================

# Basic Agent Configuration 
basic_config = {
    "config_list" : [{
        "model" : "gemini-1.5-flash",
        "api_key" : API_KEY, 
        "api_type" : "google",
        "base_url": "https://generativelanguage.googleapis.com/v1beta",
    }],
    "temperature": 0.7,  # Controls randomness (0 = deterministic, 1 = creative)
    "timeout": 30        # 30 second timeout to avoid hanging
}


# =============================================================================
# LESSON 2: Creating the Agent, using above configuration
# =============================================================================
print("\n📖 LESSON 2: Creating the Agent System")
print("-" * 40)


# Creating a simple agent
agent = ConversableAgent(
    name="Helper",                              # Give your agent a name
    system_message="You are a helpful tutor.",  # Tell the agent its role
    llm_config=basic_config,                    # Connect to Gemini
    human_input_mode="NEVER",                    # Don't ask for human input
)

print("✅ Created agent named 'Helper'")
print("-" * 40)

# =============================================================================
# LESSON 3.0: Writing a List of Message for the agents
# =============================================================================

print("-" * 42)

# Different ways to structure messages
message_examples = [
    "What is Python?",  # Simple question
    "List 3 benefits of using Python programming language.",  # Structured request
    "Write a simple 'Hello World' program in Python.",  # Code request
]

# =======================================================================================
# LESSON 3.1: Sending the Message to the Agent and receiving the message in loop
# =======================================================================================

for i, msg in enumerate(message_examples, 1):                 # starts the counting from 1 (instead of the default 0).
    print(f"\n--- Example {i} ---")
    # Create a message for the agent
    messages = [{"role": "user", "content": msg}]
    # Send the message to the agent and get a reply
    reply = agent.generate_reply(messages=messages)
    response = reply["content"]
    print(f"💬 Response: {response}")
    time.sleep(5)  # Pause between calls to avoid rate limiting

print("\n" + "=" * 50)